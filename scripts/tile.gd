...
var forest_land_atlas = Vector2i(5,0)
...
func generate_world():
	for x in range(width):
		for y in range(height):
			...
			if noise_val >= 0.0 and noise_val < 0.05:
				map_hexes.set_cell(Vector2i(x,y), source_id, sand_land_atlas)
			elif noise_val >= 0.05 and noise_val < 0.3:
				map_hexes.set_cell(Vector2i(x,y), source_id, grass_land_atlas)
			elif noise_val >= 0.3 and noise_val < 0.4:
				map_hexes.set_cell(Vector2i(x,y), source_id, forest_land_atlas)
